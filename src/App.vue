    ...
    fetch(url) {
      this.$http.get(url)
        ...
        .then((text) => {
          ...
          this.$store.dispatch('UPDATE_BOOK_META', { meta: data.attributes });
          ...
          this.$store.dispatch('UPDATE_BOOK_HTML', { html: result.join('\n\n') });
          this.$store.dispatch('UPDATE_BOOK_TREE', { tree: result.map(
            (v, k) => ({ html: v, el: htmlTree[k + 1] })) });
        });
    },
  },
  created() {
    this.$store.dispatch('UPDATE_BOOK_URL', { url: this.$route.query.url });
  },
  mounted() {
